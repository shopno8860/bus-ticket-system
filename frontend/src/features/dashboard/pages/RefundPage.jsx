...
function RefundPage() {
  ...
  const runSslSync = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${config.apiBaseUrl}${endpoints.dashboard.syncSslRefund(selectedRefund.id)}`,
        {
          method: 'POST',
          headers: {
  ...
  const runRefundAction = async (type) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const endpoint =
        type === 'approve' ? endpoints.dashboard.approveRefund(target.id) : endpoints.dashboard.rejectRefund(target.id);
