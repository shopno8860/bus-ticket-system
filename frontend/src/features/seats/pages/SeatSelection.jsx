...
const SeatSelection = () => {
  ...
  const handleContinue = () => {
    ...
    navigate(`/booking/summary?tripId=${tripId}&seats=${selectedSeats.join(",")}`);
  };
    ...
    <div className="min-h-screen bg-gray-50 md:flex md:items-center md:justify-center p-0 md:p-4">
      ...
      <div className="w-full max-w-xl mx-auto bg-white flex flex-col h-screen md:h-auto md:min-h-[600px] relative shadow-md md:rounded-xl overflow-hidden border border-gray-100">
        ...
        <div className="px-4 py-3 flex justify-between items-center border-b">
          ...
          <button onClick={() => navigate(-1)} className="text-gray-400 text-2xl px-2 hover:text-gray-600 transition-colors">
            &times;
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center gap-6">
          ...
          <div className="flex gap-5 text-[10px] font-bold uppercase text-gray-400">
             <div className="flex items-center gap-1.5">
               <div className="w-3.5 h-3.5 rounded bg-gray-100 border border-gray-200"></div> Avail
             </div>
             <div className="flex items-center gap-1.5">
               <div className="w-3.5 h-3.5 rounded bg-red-100 border border-red-200"></div> Sold
             </div>
             <div className="flex items-center gap-1.5">
        ...
        <div className="p-4 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.03)] sticky bottom-0">
          <div className="flex items-center justify-between mb-4 px-1">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Seats Selected</span>
              <span className="text-lg font-bold text-gray-900">{selectedSeats.length || 0} Seat(s)</span>
            </div>
            <div className="text-right">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Total Price</span>
...
const SeatButton = ({ seat, isSelected, onClick }) => {
  ...
  if (isBooked) {
    return (
      <button disabled className={`${base} bg-red-100 border-red-200 text-red-500/50 cursor-not-allowed`}>
        {id}
      </button>
    );
  }
  ...
  if (isSelected) {
    return (
      <button onClick={onClick} className={`${base} bg-green-600 border-green-800 text-white shadow-md active:translate-y-0.5 active:border-b-0`}>
        {id}
      </button>
    );
  }
  ...
  return (
    <button onClick={onClick} className={`${base} bg-gray-100 border-gray-200 text-gray-500 hover:bg-gray-200 active:translate-y-0.5 active:border-b-0`}>
      {id}
    </button>
  );
};
