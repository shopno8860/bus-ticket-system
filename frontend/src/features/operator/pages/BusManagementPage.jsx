...
function BusManagementPage() {
  const { data, error, loading, execute } = useFetch(getOperatorBuses);
  ...
  const handleSaveBus = async (event) => {
    ...
    try {
      if (editingBus?.id) {
        await updateOperatorBus(editingBus.id, payload);
        showToast('success', 'Bus updated successfully.');
      } else {
        await createOperatorBus(payload);
  ...
  const handleDeleteBus = async () => {
    ...
    setDeleting(true);
    try {
      await deleteOperatorBus(busToDelete.id);
  ...
  const handleGenerateSeats = async (bus) => {
    ...
    try {
      ...
      await generateOperatorBusSeats(bus.id, columnsPerRow, true);
    ...
    <div className="space-y-4 bg-slate-50 p-4">
      <section className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500">Manage your buses, capacity, and status.</p>
        </div>
        <button
          type="button"
