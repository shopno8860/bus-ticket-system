...
function RouteManagementPage() {
  const { data, error, loading, execute } = useFetch(getOperatorRoutes);
  ...
  const handleDeleteRoute = async () => {
    ...
    setDeleting(true);
    try {
      await deleteOperatorRoute(routeToDelete.id);
    ...
    <div className="space-y-4 bg-slate-50 p-4">
      <section className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500">Manage your routes between cities.</p>
        </div>
        <button
          type="button"
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        {loading ? (
          ...
          <div className="px-4 py-10 text-center">
            <p className="text-sm text-slate-500">No routes found. Add your first route to get started.</p>
          </div>
        ) : (
