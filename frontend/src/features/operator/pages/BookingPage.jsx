...
function createManualBooking(payload) {
  return apiFetch(endpoints.operator.bookings, {
    method: 'POST',
...
function BookingPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!passengerName.trim() || !passengerPhone.trim()) {
      setFormError('Passenger name and phone are required.');
      return;
    }
              ...
              <tbody>
                {bookings.slice(0, 10).map((booking) => (
                  <tr key={booking.id} className="border-b border-slate-100 text-sm hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 text-slate-700">{booking.seats?.length ?? '-'}</td>
                    <td className="px-3 py-2">
