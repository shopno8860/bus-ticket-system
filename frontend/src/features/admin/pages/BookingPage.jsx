...
function BookingPage() {
  ...
  const fetchBookings = useCallback(
    async () => apiFetch(`${endpoints.admin.bookings}?${bookingQuery}`).then((res) => res.items ?? res),
    [bookingQuery],
  );
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      ...
      const response = await fetch(`${config.apiBaseUrl}${endpoints.admin.cancelBooking(bookingToCancel.id)}`, {
        method: 'PATCH',
        headers: {
