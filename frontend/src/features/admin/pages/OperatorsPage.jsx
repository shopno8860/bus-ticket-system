...
function OperatorsPage() {
  ...
  const loadOperators = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await apiFetch(endpoints.operators.list).then((res) => res.items ?? res);
  ...
  const handleCreate = async (e) => {
    ...
    setCreating(true);
    try {
      await apiFetch(endpoints.operators.create, {
        method: 'POST',
  ...
  const handleSuspend = async (id) => {
    try {
      await apiFetch(endpoints.operators.suspend(id), { method: 'POST' });
  ...
  const handleActivate = async (id) => {
    try {
      await apiFetch(endpoints.operators.activate(id), { method: 'POST' });
