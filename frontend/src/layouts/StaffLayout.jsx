...
import { FaBars, FaChartPie, FaChevronLeft, FaChevronRight, FaSignOutAlt, FaTicketAlt, FaTimes } from 'react-icons/fa';
import { useAuth } from '../features/auth/context/AuthContext';

const staffMenuItems = [
  { to: '/staff/dashboard', label: 'Dashboard', icon: FaChartPie },
  { to: '/staff/booking', label: 'Bookings', icon: FaTicketAlt },
];

const getStaffPageTitle = (pathname) => {
  const activeItem = staffMenuItems.find((item) => pathname.startsWith(item.to));
  return activeItem?.label ?? 'Staff Dashboard';
};
...
function StaffLayout() {
  ...
  const pageTitle = useMemo(() => {
    return getStaffPageTitle(location.pathname);
  }, [location.pathname]);
      ...
      >
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-4">
            {!collapsed && (
              <h1 className="text-lg font-extrabold tracking-wide text-white">Staff Panel</h1>
            )}
          ...
          <nav className="flex-1 space-y-2 px-3 py-4">
            {staffMenuItems.map(({ to, label, icon: Icon }) => (
              <NavLink
                key={to}
                to={to}
                onClick={() => setMobileOpen(false)}
